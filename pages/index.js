...
export default function Home({ tweets, trends }) {
  ...
  // const [selectedTrend, setSelectedTrend] = useState(rank[0].name)

  return (
    <div>
      <Navbar setIsDrawerOpen={setIsDrawerOpen}></Navbar>
      <Stack direction='row' spacing={10} maxWidth={'60%'} mx='auto'>
        <TweetLine tweets={tweets} sx={{ backgroundColor: 'teal' }} />

        <Sidebar>
          <TrendList trends={trends} setOpen={setIsDrawerOpen}/>
        </Sidebar>
      </Stack>
...
export async function getStaticProps() {
  ...
  const trends = tweetsJson.results.map((r) => ({
    name: r.name,
