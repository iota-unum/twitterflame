...
export async function getOfficialTweets(idsString){
  const tweets =  client.v1.get('statuses/lookup.json', { id: idsString });
return tweets
...
const tweetScore = reply_count + quote_count
  return {id_str, created_at, name, screen_name, profile_image_url_https, full_text, tweetScore, metrics:{retweet_count, favorite_count, quote_count, reply_count},} 
})
